...
fn test_mesh_network_full_lifecycle() {
    ...
    network
        .add_path(
            "node1",
            NetworkPath::new("node1").with_hops(1).with_latency(50.0),
        )
        .unwrap();
    network
        .add_path(
    ...
    network
        .add_path(
            "node2",
            NetworkPath::new("node2").with_hops(1).with_latency(75.0),
        )
        .unwrap();
    ...
    network
        .add_path(
            "node3",
            NetworkPath::new("node3").with_hops(3).with_latency(250.0),
        )
        .unwrap();
...
fn test_path_quality_edge_cases() {
    ...
    assert_eq!(
        PathQuality::from_metrics(100.0, 1.0),
...
async fn test_messaging_service_event_flow() {
    ...
    // Queue a message
    service
        .queue_message(Message::new("alice", "bob", "Queued message"))
        .unwrap();
    ...
    service
        ...
        .unwrap();
    assert_eq!(
        service.history.get(msg.id).unwrap().content,
        "Edited content"
    );
...
fn test_destination_builder_patterns() {
    ...
    // Full builder
    let dest = Destination::new("hash4").with_name("Full Node").trusted();
...
fn test_full_chat_workflow() {
    ...
    history
        .edit_message(msg1.id, "Hey @bob, are you free to chat?")
        .unwrap();
